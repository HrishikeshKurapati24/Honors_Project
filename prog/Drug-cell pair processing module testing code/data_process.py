...
def process(drug_feature, genomics_feature, epigenomics_feature, transcriptomics_feature,
            proteomics_feature, metabolomics_feature, pathway_feature, similarity_feature,
            data_new, nb_celllines, nb_drugs, independent_test=False, test_ratio=0.1,
            data_split_seed=0, calc_pe=False, physicochemical_feature=None, k_folds=None, current_fold=0):
    ...
    # 1. Get atom feature shape from the first drug
    first_drug_val = next(iter(drug_feature.values()))
    atom_shape = first_drug_val[0].shape[-1]

    # 2. Extract features in order
    drug_data = [[] for _ in range(len(ordered_drug_ids))]
    for i, drug_id in enumerate(ordered_drug_ids):
        if drug_id in drug_feature:
            feat_mat, adj_list, _ = drug_feature[drug_id]
            drug_data[i] = CalculateGraphFeat(feat_mat, adj_list)
        else:
            raise ValueError(f"Drug ID {drug_id} found in map but not in features!")
    ...
    if similarity_feature is not None:
        ...
        similarity_feature = similarity_feature.loc[ordered_cell_ids]

    # Standardizing Omics Features is NOT needed as data is already Z-score normalized in Processed data folder
    
    # Tensors - convert to torch tensors
    genomics = torch.from_numpy(genomics_feature.values.astype(np.float32)) if genomics_feature is not None else None
    epigenomics = torch.from_numpy(epigenomics_feature.values.astype(np.float32)) if epigenomics_feature is not None else None
    transcriptomics = torch.from_numpy(transcriptomics_feature.values.astype(np.float32)) if transcriptomics_feature is not None else None
    proteomics = torch.from_numpy(proteomics_feature.values.astype(np.float32)) if proteomics_feature is not None else None
    metabolomics = torch.from_numpy(metabolomics_feature.values.astype(np.float32)) if metabolomics_feature is not None else None
    pathway = torch.from_numpy(pathway_feature.values.astype(np.float32)) if pathway_feature is not None else None
    similarity_tensor = torch.from_numpy(similarity_feature.values.astype(np.float32)) if similarity_feature is not None else None
    ...
    if physicochemical_feature is not None:
        # Get dimension from the first available feature
        sample_feat = next(iter(physicochemical_feature.values()))
        ...
        for drug_id in ordered_drug_ids:
            if drug_id in physicochemical_feature:
                ...
                physicochemical_list.append(np.zeros(feat_dim, dtype='float32'))
        ...
        physicochemical_tensor = torch.zeros(nb_drugs, 63, dtype=torch.float32)
    ...
    # Positive/negative edges for labels (using all pairs for ground truth mapping)
    pos_edge = allpairs[allpairs[:, 2] == 1, 0:2]
    pos_edge_proc = pos_edge.copy()
    pos_edge_proc[:, 1] -= nb_celllines

    label_pos = coo_matrix((np.ones(pos_edge_proc.shape[0]), (pos_edge_proc[:, 0], pos_edge_proc[:, 1])), shape=(nb_celllines, nb_drugs)).toarray()
    label_pos = torch.from_numpy(label_pos).type(torch.FloatTensor).view(-1)

    # Train edges (return as directed Cell->Drug pairs with global indices for graph construction)
    train_edge = train

    return (drug_set, cellline_set, train_edge, label_pos, train_mask, val_mask, test_mask,
            atom_shape, physicochemical_tensor, similarity_tensor)