...
class NodeRepresentation(nn.Module):
    def __init__(self, gcn_layer, dim_gexp, dim_methy, output, units_list=[256, 256, 256], use_relu=True, use_bn=True,
        ...
        print(f"  - Mutation: {'Enabled' if self.use_mutation else 'Disabled'}")
        print(f"  - Gene Expression: {'Enabled' if self.use_gexpr else 'Disabled'}")
        print(f"  - Methylation: {'Enabled' if self.use_methylation else 'Disabled'}")
    ...
    def forward(self, drug_feature, drug_adj, ibatch, mutation_data, gexpr_data, methylation_data):
        ...
        if self.use_mutation and mutation_data is None:
            raise ValueError("Mutation is enabled but no data provided.")
        if self.use_gexpr and gexpr_data is None:
            raise ValueError("Gene expression is enabled but no data provided.")
        if self.use_methylation and methylation_data is None:
            raise ValueError("Methylation is enabled but no data provided.")
