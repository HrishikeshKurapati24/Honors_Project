...
class NodeRepresentation(nn.Module):
    ...
    def forward(self, drug_feature, drug_adj, ibatch, mutation_data, gexpr_data, methylation_data):
        ...
        ibatch = ibatch.to(device)
        if mutation_data is not None: mutation_data = mutation_data.to(device)
        if gexpr_data is not None: gexpr_data = gexpr_data.to(device)
        if methylation_data is not None: methylation_data = methylation_data.to(device)
...
class GraphCDR(nn.Module):
    ...
    def forward(self, drug_feature, drug_adj, ibatch, mutation_data, gexpr_data, methylation_data, edge):
        # Ensure omics data is on target device
