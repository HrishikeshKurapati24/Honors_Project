...
def _load_deepttc_module(root_dir: str):
    ...
    return load_external_module(
        module_name="benchmark_deepttc_shared_graph_model",
...
def _build_scope(dataset: Dict, scope_pairs: pd.DataFrame, train_pairs: pd.DataFrame, device: torch.device, top_k: int) -> Dict:
    ...
    if cached is None:
        ...
        atom_dim = next(iter(graph_features.values()))[0].shape[1]
        max_tokens = 50
        drug_tokens = np.zeros((len(drug_ids), max_tokens, atom_dim), dtype=np.float32)
        attention_mask = np.zeros((len(drug_ids), max_tokens), dtype=np.int64)
        for drug_idx, drug_id in enumerate(drug_ids):
            feat_mat = graph_features[drug_id][0].astype(np.float32)
            limit = min(max_tokens, feat_mat.shape[0])
            drug_tokens[drug_idx, :limit] = feat_mat[:limit]
            attention_mask[drug_idx, :limit] = 1

        graph_edges = build_shared_similarity_graphs(
        ...
        cached = {
            ...
            "drug_tokens": torch.from_numpy(drug_tokens),
            "attention_mask": torch.from_numpy(attention_mask),
            "cell_edge_index": graph_edges[("cell", "similar_to", "cell")],
            "drug_edge_index": graph_edges[("drug", "similar_to", "drug")],
            "response_edge_index": response_edge_index,
        }
    ...
    return {
        ...
        "expression": cached["expression"].to(device),
        "drug_tokens": cached["drug_tokens"].to(device),
...
def _encode_scope_embeddings(model, scope: Dict) -> Tuple[torch.Tensor, torch.Tensor]:
    drug_embeddings = model.encode_drugs(
        scope["drug_tokens"],
        scope["attention_mask"],
        scope["drug_edge_index"],
    )
    cell_embeddings = model.encode_cells(
        scope["expression"],
        scope["cell_edge_index"],
    )
    return drug_embeddings, cell_embeddings


def _predict_logits(model, scope: Dict, pair_indices: torch.Tensor) -> torch.Tensor:
    drug_embeddings, cell_embeddings = _encode_scope_embeddings(model, scope)
...
def _predict_scores_chunked(
    ...
    with torch.autocast(device_type="cuda", dtype=torch.float16, enabled=use_amp):
        ...
        for start in range(0, pair_indices.shape[0], batch_size):
            logits = model.predict_pair_logits(
                drug_embeddings,
                cell_embeddings,
...
def run(
    ...
    for fold in resolve_fold_ids(split_dir, fold_ids):
        ...
        for epoch in range(epochs):
            ...
            model.eval()
            with torch.no_grad():
                val_scores = _predict_scores_chunked(
                    model,
                    val_scope,
                    val_indices,
                    batch_size=batch_size,
        ...
        if best_state is None:
            raise RuntimeError("DeepTTC strict runner failed to capture a best checkpoint")

        model.load_state_dict(best_state)
        model.eval()
        with torch.no_grad():
            test_scores = _predict_scores_chunked(
                model,
                test_scope,
                test_indices,
                batch_size=batch_size,
