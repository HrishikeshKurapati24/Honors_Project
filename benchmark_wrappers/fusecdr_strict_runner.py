...
def _load_fusecdr_module(root_dir: str):
    ...
    return load_external_module(
        module_name="benchmark_fusecdr_strict_main",
...
def _prepare_runtime(processed, device: torch.device, top_k: int) -> Dict:
    return {
        "omics_data_device": {
            category: {subtype: tensor.to(device) for subtype, tensor in subtype_map.items()}
            for category, subtype_map in processed.omics_tensors.items()
        },
        ...
        "global_edge_index_dict": build_shared_similarity_graphs(
            cell_similarity_features=processed.similarity_tensor,
            drug_similarity_features=processed.physicochemical_tensor,
            top_k=top_k,
            device=device,
        ),
    }
    ...
    for fold in resolve_fold_ids(split_dir, fold_ids):
        ...
        if protocol == PROTOCOL_RANDOM:
            ...
            processed_train = module.process_flexible(
                ...
                k_folds=5,
                current_fold=fold - 1,
                data_split_seed=seed + fold * 1000,
                drug_batch_size=0,
        ...
        train_runtime = _prepare_runtime(processed_train, runtime_device, top_k)
        val_runtime = _prepare_runtime(processed_val, runtime_device, top_k)
        test_runtime = _prepare_runtime(processed_test, runtime_device, top_k)
