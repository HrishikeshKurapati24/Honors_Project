...
def _load_fusecdr_module(root_dir: str):
    ...
    return load_external_module(
        module_name="benchmark_fusecdr_smiles_main",
...
def _build_scope(dataset: Dict, cell_ids: List[str], drug_ids: List[str], selected_omics: List[str]) -> SmilesScope:
    ...
    for selector in selected_omics:
        if "_" in selector:
            ...
            category, subtype = selector, selector
        frame = subset_frame(tables[selector], cell_ids)
        omics_tensors.setdefault(category, {})[subtype] = torch.from_numpy(frame.to_numpy(dtype="float32"))

    similarity = subset_frame(tables["similarity"], cell_ids)
    phys = subset_frame(tables["physicochemical"], drug_ids)
    token_ids = subset_frame(aux["smiles_token_ids"], drug_ids)
    masks = subset_frame(aux["smiles_attention_mask"], drug_ids)
    ...
    return SmilesScope(
        ...
        drug_similarity_tensor=torch.from_numpy(phys.to_numpy(dtype="float32")),
        token_tensor=torch.from_numpy(token_ids.to_numpy(dtype="int64")),
        mask_tensor=torch.from_numpy(masks.to_numpy(dtype="int64")),
    )
...
def _build_edge_dict(module, scope: SmilesScope, train_pairs: pd.DataFrame, device: torch.device, top_k: int):
...
def _predict_pairs(model, scope: SmilesScope, pair_indices: torch.Tensor, edge_dict, omics_data_device, device: torch.device):
    out = model(
        drug_feature=(scope.token_tensor.to(device), scope.mask_tensor.to(device)),
        drug_adj=None,
        ibatch=None,
...
def run(
    ...
    selected_omics = prepared_metadata.get("omics_for_fusecdr", ["transcriptomics_expression"])
    ...
    for fold in resolve_fold_ids(split_dir, fold_ids):
        ...
        if protocol == "random":
            ...
            train_scope = _build_scope(dataset, all_cells, all_drugs, selected_omics)
            ...
            train_scope = _build_scope(dataset, train_cells, train_drugs, selected_omics)
            val_scope = _build_scope(dataset, val_cells, val_drugs, selected_omics)
            test_scope = _build_scope(dataset, test_cells, test_drugs, selected_omics)
        ...
        train_pairs_idx, train_labels = _pairs_to_local_tensors(train_pairs_scope, train_scope.cell_ids, train_scope.drug_ids, runtime_device)
        val_pairs_idx, val_labels = _pairs_to_local_tensors(val_pairs_scope, val_scope.cell_ids, val_scope.drug_ids, runtime_device)
        test_pairs_idx, test_labels = _pairs_to_local_tensors(test_pairs_scope, test_scope.cell_ids, test_scope.drug_ids, runtime_device)
        ...
        for epoch in range(epochs):
            ...
            optimizer.zero_grad()
            train_out = model(
                drug_feature=(train_scope.token_tensor.to(runtime_device), train_scope.mask_tensor.to(runtime_device)),
                drug_adj=None,
                ibatch=None,
            ...
            model.eval()
            with torch.no_grad():
                val_scores = _predict_pairs(model, val_scope, val_pairs_idx, val_edge_dict, val_omics, runtime_device)
        ...
        if best_state is None:
            raise RuntimeError("FUSECDR smiles runner failed to capture a best checkpoint")

        model.load_state_dict(best_state)
        model.eval()
        with torch.no_grad():
            test_scores = _predict_pairs(model, test_scope, test_pairs_idx, test_edge_dict, test_omics, runtime_device).cpu()
    ...
    return save_model_outputs(
        ...
        metadata={
            ...
            "drug_similarity_source": prepared_metadata.get("drug_similarity_source", "physicochemical"),
            "config": {
                "lr": lr,
