...
def _load_redcdr_model(root_dir: str):
    ...
    return load_external_module(
        module_name="benchmark_redcdr_model",
...
def _build_scope(dataset, graph_dir: str, scope_pairs: pd.DataFrame, train_pairs: pd.DataFrame, val_pairs: pd.DataFrame, test_pairs: pd.DataFrame, device: torch.device) -> Dict:
    ...
    allpairs = build_redcdr_allpairs(cell_ids, drug_ids, scope_pairs)
...
def run(
    ...
    for fold in resolve_fold_ids(split_dir, fold_ids):
        ...
        if protocol == "random":
            ...
            train_scope = _build_scope(dataset, graph_dir, train_pairs, train_pairs, _empty_pairs(), _empty_pairs(), runtime_device)
            val_scope = _build_scope(dataset, graph_dir, _concat_pairs(train_pairs, val_pairs), train_pairs, val_pairs, _empty_pairs(), runtime_device)
            test_scope = _build_scope(dataset, graph_dir, _concat_pairs(train_pairs, test_pairs), train_pairs, _empty_pairs(), test_pairs, runtime_device)
