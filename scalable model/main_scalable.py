...
Locked scalable training script for a single best SOUL-CDR architecture:
- Drug GNN: GIN
...
from data_scalable import dataload_scalable, list_available_omics, process_scalable
from model_scalable import EncoderConfig, OmicsEncoderRegistry, SOULCDR
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Locked scalable SOUL-CDR trainer (best single architecture)."
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to a SOULCDR-compatible dataset directory. Defaults to final_dataset at the repository root.",
    )
    parser.add_argument(
        "--omics",
        nargs="+",
...
def main():
    ...
    run_name = args.run_name if args.run_name else f"scalable_best_{timestamp}"
    ...
    loaded = dataload_scalable(dataset_root, selected_omics=args.omics)
    print(
    ...
    for fold in range(args.k_fold):
        print("\n" + "=" * 80)
        print(f"Fold {fold + 1}/{args.k_fold}")
        print("=" * 80)

        processed = process_scalable(
            loaded=loaded,
            k_folds=args.k_fold,
            current_fold=fold,
        ...
        ckpt_path = os.path.join(run_dir, f"fold_{fold + 1}_best.pt")
        ...
        history_path = os.path.join(run_dir, f"fold_{fold + 1}_history.csv")
        write_csv(
            history_path,
            fieldnames=[
                "epoch",
        ...
        pred_path = os.path.join(run_dir, f"fold_{fold + 1}_test_predictions.npz")
    ...
    with open(os.path.join(run_dir, "summary.txt"), "w") as f:
        f.write("Locked Scalable SOUL-CDR Results\n")
        f.write("=" * 40 + "\n")
