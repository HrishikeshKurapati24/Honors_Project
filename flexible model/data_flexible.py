...
def _is_supported_omics_pair(category: str, subtype: str) -> bool:
    if (category, subtype) in SUPPORTED_OMICS_CATEGORY_SUBTYPE:
        return True
...
def _discover_omics_entries(dataset_root: str) -> List[Dict[str, str]]:
    ...
    for path in csv_paths:
        ...
        if not _is_supported_omics_pair(category, subtype):
            continue
...
def list_available_omics(dataset_root: str) -> List[Dict[str, str]]:
    """
    Return discovered omics selectors from a prepared FUSECDR dataset directory.
...
def dataload_flexible(
    dataset_root: str, selected_omics: Optional[List[str]] = None
) -> LoadedFlexibleData:
    """
    Load and align data from a FUSECDR-compatible dataset directory.
...
def process_flexible(
    ...
    # 2) Omics tensors in nested dict format expected by flexible cell module
    omics_tensors: Dict[str, Dict[str, torch.Tensor]] = {}
    for category, subtype_map in omics_features.items():
