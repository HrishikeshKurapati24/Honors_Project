...
def process(drug_feature, mutation_feature, gexpr_feature, methylation_feature, data_new, nb_celllines, nb_drugs, data_split_seed=666):
    ...
    atom_shape=drug_feature.iloc[0].iloc[0].shape[-1]
    ...
    if(use_independent_testset == True):
        edge_mask = cmask(len(allpairs), 0.1, data_split_seed)
        train = allpairs[edge_mask][:, 0:3]
        test = allpairs[~edge_mask][:, 0:3]
    else:
        CV_edgemask = cmask(len(allpairs), 0.1, data_split_seed)
