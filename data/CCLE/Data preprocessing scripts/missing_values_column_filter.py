...
output_dir = "./column_filtered_omics"  # directory to save cleaned files
...
def clean_file(file_path, output_path, threshold):
    ...
    cleaned_df = df.dropna(axis=1, thresh=min_non_missing)
    cleaned_df.to_csv(output_path, index=False)
    print(f"[{os.path.basename(file_path)}]  Columns before: {len(df.columns)} | after: {len(cleaned_df.columns)}")
    return cleaned_df
...
print(f"Cleaned files saved in: {os.path.abspath(output_dir)}")